...
def mergefile_graph(df_basic,df_incremental):
  ...
  df_basic.to_csv(r'C:\Users\pc\Desktop\College\BTECH\FDP-Distributed\Dataset\record.csv',index = False)
...
def mergefile_representative(dftrain,dftest):
  ...
  dftrain.to_csv(r'C:\Users\pc\Desktop\College\BTECH\FDP-Distributed\Dataset\record.csv',index = False)
  #(dftrain.groupby(['CNumber'],as_index = False).mean()).to_csv('C:\Users\pc\Desktop\College\BTECH\FDP-Distributed\Dataset\record.csv')
      ...
      for j in range(i+1,len(df)):
        if(df.Flag[j]==False):
          ...
          if close<=1:
            ...
            df1.to_csv(r'C:\Users\pc\Desktop\College\BTECH\FDP-Distributed\Dataset\record.csv')
    ...
    for j in range(len(dftest)):
      if(dftest.Flag[j]==False):
        ...
        if close<=1:
          ...
          df2.to_csv(r'C:\Users\pc\Desktop\College\BTECH\FDP-Distributed\Dataset\record.csv')
        else:
          outlier.append(j)
